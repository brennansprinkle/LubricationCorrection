...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    incr = 20;  %---- Steps of cone increments
    coneincr = coneRadius/incr;    
    cfact = 1.5; %used to be 2;
    [coneX, coneY, coneZ] = cylinder(cfact*cylinderRadius:-coneincr:0,50);  %---------- CONE 
    ...
    stemfuzz = (1 + 0.002*randn(size(stemX)));
    headfuzz = (1 + 0.000*randn(size(headX)));
    %************************************************************************************************************    
    hStem = surf(stemfuzz.*stemX, stemfuzz.*stemY, stemZ, 'FaceColor', colorCode, 'EdgeColor', 'none'); 
    hold on;  
    hHead = surf(headfuzz.*headX, headfuzz.*headY, headfuzz.*headZ, 'FaceColor', colorCode, 'EdgeColor', 'none');
    hold on;
    sphereX(sphereX<0) = NaN;
    sphereRadius = 1.5*coneRadius;
    hBack = surf(0*sphereX+min(stemX(:)),sphereRadius*sphereY+Y,sphereRadius*sphereZ+Z, 'FaceColor', colorCode, 'EdgeColor', 'none');
    hold on;
    
    if nargout==1   
        arrowHandle = [hStem, hHead,hBack]; 
    end