...
if __name__ == '__main__':
    ...
    debye_delta = read.debye_length_wall
    debye_length_delta = 2.0*debye_delta/np.log(1.0e1)
    ...
    for n in range(n_steps):
      ...
      mag_sign = 0.0
      if time_s > 0.0:
	mag_sign = -0.8 #negative -3.0 for helix
      ...
      FT_calc = partial(multi_bodies_functions.force_torque_calculator_sort_by_bodies, 
                                               ...
                                               spring_k = 10.0, #3.5,
                                               spring_l = 2.1*a,
                                               k_bend = 0.3,  #1.1 gave a lasso kinda for a bit but hen kinked #0.6, #1.1 #0.0317
      ...
      print("Force calc time : %s" %dt1)
      FT_sum = np.sum(force_torque.flatten())
      print("Force sum : %s" %FT_sum)
      #print force_torque
      
      t0 = time.time()
      reject_wall, reject_jump = LSolv.Update_Bodies_no_lub(force_torque.flatten()) 
      dt1 = time.time() - t0
