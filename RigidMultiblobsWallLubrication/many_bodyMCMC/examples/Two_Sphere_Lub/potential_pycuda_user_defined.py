...
__device__ void blob_blob_potential(double &u,
  ...
  if(i != j){
    ...
    u += eps * 0.5 * (r - debye_length) * (r - debye_length);
    
    if(r < 2.0*blob_radius){
      u += eps + eps * (2.0*blob_radius - r) / 0.01 / blob_radius;
    }
    else{
      u += eps * exp(-(r - 2.0*blob_radius) / 0.01 / blob_radius);
    }
...
__global__ void potential_from_position_blobs(const double *x,
  ...
  else
  {
    ...
    u = 1e+05*(-x[ioffset+2] + 1 + blob_radius); 
  }
  //IF END
