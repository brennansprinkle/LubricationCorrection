...
for k = [1 2 3 4]
    ...
    elseif k == 2
        B = dlmread(['./Pair_Mob_Data/mob_matrix_642.txt']);
    elseif k == 3
        B = dlmread(['./Pair_Mob_Data/mob_matrix_12.txt']);
    elseif k == 4
...
for p = 1:m
    p
    M_lub = reshape(A(p,:),12,12);
    M_mb = reshape(B(p,:),12,12);
        h(k) = plot(x(p),norm(M_lub-M_mb),syms{k},'markersize',10,'color',cols(k,:));
    hold all
    axis tight
