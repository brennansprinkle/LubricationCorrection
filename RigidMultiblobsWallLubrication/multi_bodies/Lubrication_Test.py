...
while found_functions is False:
  try:
    ...
    from quaternion_integrator.quaternion_integrator_lubrication_test import QuaternionIntegrator
...
def calc_K_matrix(bodies, Nblobs):
  ...
  '''
  K = np.zeros((3*Nblobs, 3*len(bodies)))
  offset = 0
  for k, b in enumerate(bodies):
    K[3*offset:3*(offset+b.Nblobs), 3*k:3*k+3] = np.identity(3)
    offset += b.Nblobs
  return K
...
def K_matrix_vector_prod(bodies, vector, Nblobs, K_bodies = None):
  ...
  v = np.reshape(vector, (len(bodies) * 3))
  ...
  for k, b in enumerate(bodies):
    if K_bodies is None:
      ...
      K = K_bodies[k] 
    result[offset : offset+1] = np.reshape(np.dot(K, v[3*k : 3*(k+1)]), (b.Nblobs, 3))
    offset += 1    
  return result
...
def K_matrix_T_vector_prod(bodies, vector, Nblobs, K_bodies = None):
  ...
  for k, b in enumerate(bodies):
    if K_bodies is None:
      ...
      K = K_bodies[k] 
    result[k : k+1] = np.dot(K.T, v[3*offset : 3*(offset+1)])
    offset += 1    

  result = np.reshape(result, (len(bodies), 3))
  return result
...
def linear_operator_rigid(vector, bodies, r_vectors, eta, a, K_bodies = None, *args, **kwargs):
  ...
  K_times_U = K_matrix_vector_prod(bodies, v[Nblobs : Nblobs+len(bodies)], Nblobs, K_bodies = K_bodies) 
...
def build_block_diagonal_preconditioners_det_stoch(bodies, r_vectors, Nblobs, eta, a, *args, **kwargs):
  ...
  def block_diagonal_preconditioner(vector, bodies = None, mobility_bodies = None, mobility_inv_blobs = None, K_bodies = None, Nblobs = None, *args, **kwargs):
    ...
    for k, b in enumerate(bodies):
      ...
      F = vector[3*Nblobs + 3*k : 3*Nblobs + 3*(k+1)]
      ...
      # 4. Set result
      result[3*Nblobs + 3*k : 3*Nblobs + 3*(k+1)] = Y
      offset += b.Nblobs
    return result
...
def block_diagonal_preconditioner(vector, bodies, mobility_bodies, mobility_inv_blobs, Nblobs):
  ...
  for k, b in enumerate(bodies):
    ...
    # 2. Compute rigid body velocity
    F = vector[3*Nblobs + 3*k : 3*Nblobs + 3*(k+1)]
    Y = np.dot(mobility_bodies[k], -F - np.dot(np.identity(3).T, Lambda_tilde))

    # 3. Solve M*Lambda = (slip + K*Y)
    Lambda = np.dot(mobility_inv_blobs[k], slip + np.dot(np.identity(3), Y))
...
if __name__ == '__main__':
  ...
  integrator.eta = eta
  integrator.a = a
  integrator.first_guess = np.zeros(Nblobs*3 + num_bodies*3)
