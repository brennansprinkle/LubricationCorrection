...
if __name__ == '__main__':
  ...
  for dist in [0.025, 0.25, 1.0, 4.0, 6.0]:
	...
	integrator.advance_time_step(dt, step = step)
      ...
      if d_count == 0:
	f = open("tetra_particle_velocities_12_dist_" + str(dist) + ".dat", "w")
	np.savetxt(f, np.array([height]), newline=" ")
	f = open("tetra_particle_velocities_12_dist_" + str(dist) + ".dat", "a")
	np.savetxt(f, integrator.velocities, newline=" ")
	f.write('\n')
      else:
	f = open("tetra_particle_velocities_12_dist_" + str(dist) + ".dat", "a")
	np.savetxt(f, np.array([height]), newline=" ")
