...
def bodies_external_force_torque(bodies, r_vectors, *args, **kwargs):
  ...
  for k, b in enumerate(bodies):
    force_torque[2*k,:] += -g * np.array([0., 0., 1.0])
    force_torque[2*k+1,:] += (omega*8.0*np.pi*eta*a**3) * np.array([0., 1.0, 0.])
    # Add wall interaction
    r_b = bodies[k].location
    h = r_b[2]
    if h > a:
      force_torque[2*k,:] += (repulsion_strength_wall / debye_length_wall) * np.exp(-(h-a)/debye_length_wall)* np.array([0., 0., 1.0])
    else:
      force_torque[2*k,:] += (repulsion_strength_wall / debye_length_wall)* np.array([0., 0., 1.0])
  return force_torque
...
def body_body_force_torque(r, quaternion_i, quaternion_j, *args, **kwargs):
  ...
  if r_norm > 2*a:
    force_torque_bb[0,:] = -((eps / b) * np.exp(-(r_norm-2*a) / b) / np.maximum(r_norm, np.finfo(float).eps)) * r 
  else:
    force_torque_bb[0,:] = -((eps / b) / np.maximum(r_norm, np.finfo(float).eps)) * r
  return force_torque_bb
...
def calc_body_body_forces_torques_python(bodies, r_vectors, *args, **kwargs):
  ...
  print 'body body force: '+ str((end - start)) 
